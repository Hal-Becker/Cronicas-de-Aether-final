...
func update_quest( _title : String, _completed_step : String = "", _is_complete : bool = false ) -> void:
	...
	if quest_index == -1:
		...
		PlayerHud.queue_notification( "Misión Iniciada", _title )
		pass
	else:
		...
		if q.is_complete == true:
			PlayerHud.queue_notification( "Misión Completada!", _title )
			disperse_quest_rewards( find_quest_by_title( _title ) )
			
		else:
			PlayerHud.queue_notification( "Actualización de Misión", _title + ": " + _completed_step )
	pass
...
func disperse_quest_rewards( _q : Quest ) -> void:
	...
	PlayerHud.queue_notification( "Recompensas de Misión Recibidas!", _message )
	pass
