...
func update_summary() -> void:
	if linked_quest == null:
		...
		return
	settings_summary = "Actualización de misión:\nMisión: " + linked_quest.title + "\n"
	if check_type == CheckType.HAS_QUEST:
		settings_summary += "Comprobando si el jugador tiene una misión."
	elif check_type == CheckType.QUEST_STEP_COMPLETE:
		settings_summary += "Comprobando si el jugador ha completado el paso: " + get_step()
	elif check_type == CheckType.ON_CURRENT_QUEST_STEP:
		settings_summary += "Compruebe si el jugador está en el paso: " + get_step()
	elif check_type == CheckType.QUEST_COMPLETE:
		settings_summary += "Comprobando si la misión está completa."
	pass
