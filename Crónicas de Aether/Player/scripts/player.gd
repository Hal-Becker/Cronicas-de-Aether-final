...
const PLAYER_FIREBALL = preload("res://Enemies/Phantom/Fireball.tscn")
...
var mana_regen : float = 20  # por segundo
var fire_cost : float = 25
...
func _process( _delta ):
	...
	mana += mana_regen * _delta
	mana = clamp(mana, 0, max_mana)
	if Input.is_action_just_pressed("magic"):
		print("DISPARO PRESIONADO")
		try_shoot_fireball()
	pass
...
func try_shoot_fireball():
	if not PlayerManager.has_fire_power:
		return
	...
	var fireball = PLAYER_FIREBALL.instantiate()
	get_tree().current_scene.add_child(fireball)
	
	var dir = cardinal_direction
	
	fireball.global_position = global_position + dir * 30 
